...
| `--color-bg-header` | `#0B0F12` | 80% | Header background (semi-transparent) |
...
| `--spacing-2xl` | 80px | Hero content vertical gap |
| `--spacing-3xl` | 96px | Hero section padding top/bottom |
| `--spacing-page-x` | 144px | Main horizontal page gutter (desktop) |
...
│  └───────────────────────────────────────────────────────────────────────────────────────────────┘   │
│                                                                                                       │
│  ┌──────────────── B_Bìa (h:845px, px:144px, py:96px, flex-col, gap:120px, y:88) ─────────────┐     │
│  │                                                                                              │     │
...
| gap | 238px | `gap-[238px]` |
| position | absolute, top:0 | `absolute top-0 left-0 right-0` |
| z-index | 1 | `z-10` |
...
├── C_Keyvisual (absolute, inset-0, z-0) — Background artwork image
├── Rectangle57 (absolute, inset-0, z-1) — Horizontal gradient overlay
├── Cover (absolute, top-[138px], bottom-0, z-1) — Bottom gradient overlay
│
├── A_Header (absolute, top-0, w-full, h-20, px-36, py-3, bg-[#0B0F12]/80, z-10)
│   ├── A.1_Logo (w-[52px], h-14) — SAA logo image
│   └── A.2_Language (w-[108px], h-14, p-4, rounded, cursor-pointer, flex items-center justify-between)
│       ├── VN Flag icon (24×24)
│       ├── "VN" text (Montserrat 700 16px #FFF)
│       └── ChevronDown icon (24×24)
│
├── B_Bìa (absolute, top-[88px], w-full, px-36, py-24, flex flex-col gap-[120px])
│   └── Frame487 (w-[1152px], flex flex-col gap-20, items-start)
│       ├── B.1_KeyVisual (w-[451px], h-[200px]) — ROOT FURTHER image (next/image)
│       └── Frame550 (pl-4, flex flex-col gap-6)
│           ├── B.2_Content (max-w-[480px], Montserrat 700 20px lh:40px #FFF)
│           │   "Bắt đầu hành trình của bạn cùng SAA 2025.\nĐăng nhập để khám phá!"
│           ├── B.3_Login (w-[305px], h-[60px], bg-[#FFEA9E], rounded-lg, px-6 py-4, flex items-center)
...
│
└── D_Footer (absolute, bottom-0, w-full, px-[90px], py-10, border-t border-[#2E3940], flex justify-between items-center)
...
| B_Bìa | padding-x | `px-4` (16px), `py-16` |
| B_Bìa | flex gap | `gap-10` |
...
> All icons MUST be implemented as Icon Components, not `<img>` tags or inline SVG files.
...
| Header | `662:14391` | `absolute top-0 w-full h-20 px-36 bg-[#0B0F12]/80 flex justify-between items-center z-10` | `<Header />` |
| Logo | `I662:14391;186:2166` | `w-[52px] h-[48px]` | `<Image />` (next/image) |
...
| Hero Section | `662:14393` | `absolute top-[88px] w-full px-36 py-24 flex flex-col` | `<HeroSection />` |
| ROOT FURTHER Image | `662:14395` | `w-[451px] h-[200px]` | `<Image />` (next/image) |
| Hero Description | `662:14753` | `font-bold text-xl leading-10 text-white tracking-[0.5px]` | `<p>` |
| Login Button | `662:14426` | `flex items-center gap-2 bg-[#FFEA9E] rounded-lg py-4 px-6 w-[305px] h-[60px] hover:shadow-lg hover:-translate-y-0.5 transition-all` | `<LoginButton />` |
...
| Footer | `662:14447` | `absolute bottom-0 w-full px-[90px] py-10 border-t border-[#2E3940] flex justify-between items-center` | `<Footer />` |
...
- Google icon in login button MUST be an Icon Component.
