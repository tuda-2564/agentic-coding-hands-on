...
Users will provide `fileKey` and `frameId` from Figma to generate test cases (e.g., "generate test cases for fileKey='abc123' frameId='456:789'").

Working principle: Generate test cases for each screen sequentially, ask user when necessary.

## Test Case Generation Process

### STEP 1: CONVERT FIGMA TO SPEC

**Input:**
- `fileKey` - Figma file key (e.g., "abc123xyz")
- `frameId` - Frame ID (e.g., "123:456")

**Action:**
1.  **Call MCP Tool:**
    ```
    get_frame(fileKey, frameId)
    ```
...
1.  **Call MCP Tool:**
    ```
    list_design_items(fileKey, frameId)
    ```
...
4) **Write Output**
- Path: `.momorph/testcases/{fileKey}/{frameId}-{screen-name}.csv`
- fileKey, frameId, and screen-name MUST MATCH EXACTLY (character-for-character) the input context.
