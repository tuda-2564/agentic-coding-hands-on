...
Users will provide `fileKey` and `frameId` from Figma (e.g., "update test cases for fileKey='abc123' frameId='456:789'").
...
1.  **Call MCP Tool:**
    ```
    list_frame_spec_diffs(fileKey, frameId)
    ```
...
1.  **Call MCP Tool:**
    ```
    get_frame_test_cases(fileKey, frameId)
    ```
